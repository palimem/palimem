#!/usr/bin/env bash
# windsurf-smoke.sh — non-interactive smoke test for the Windsurf MCP integration.
...
echo "== Windsurf smoke: memory_status via stdio MCP =="
...
send({"jsonrpc": "2.0", "id": 1, "method": "initialize", "params": {
    ...
    "capabilities": {},
    "clientInfo": {"name": "windsurf-smoke", "version": "1.0"},
}})
init_resp = recv()
...
proc.terminate()
proc.wait(timeout=5)
print("windsurf-smoke: OK — memory_status responded, all 11 tools present")
sys.exit(0)
PY

echo "== WINDSURF SMOKE PASSED =="
