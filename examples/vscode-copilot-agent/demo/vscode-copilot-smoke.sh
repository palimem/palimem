#!/usr/bin/env bash
# vscode-copilot-smoke.sh — MCP stdio smoke for VS Code / Copilot Agent integrations.
set -euo pipefail
...
WORKSPACE="$(mktemp -d)"
SMOKE_NAME="${SMOKE_NAME:-vscode-copilot-smoke}"
...
echo "== ${SMOKE_NAME}: memory_status via stdio MCP =="
if [ ! -f "$MCP_SCRIPT" ]; then
  echo "ERROR: $MCP_SCRIPT not found — run npm install in $APP" >&2
  exit 1
fi

SMOKE_NAME="$SMOKE_NAME" "$PYTHON" - <<PY
import json, os, subprocess, sys

mcp_script = "$MCP_SCRIPT"
data_dir = "$DATA_DIR"
smoke_name = os.environ["SMOKE_NAME"]
...
proc.terminate()
proc.wait(timeout=5)
print(f"{smoke_name}: OK — memory_status responded, all 11 tools present")
PY

echo "== ${SMOKE_NAME} PASSED =="
