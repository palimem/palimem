#!/usr/bin/env bash
# Phase 6 readiness gate — current release gate for spec v1.7.0.
set -euo pipefail
...
"$PYTHON" "$ROOT/benchmarks/run_benchmarks.py" --strict >/dev/null && pass "benchmarks" || fail "benchmarks"
...
echo ""
if [ $FAIL -eq 0 ]; then
  echo "ALL PHASE 6 READINESS CHECKS PASSED"
else
  echo "SOME PHASE 6 READINESS CHECKS FAILED"
  exit 1
fi
